...
@app.get("/debug/kg")
async def debug_kg(q: str, kg: KnowledgeGraphService = Depends(get_kg)):
    status = kg.status()
    if not status["enabled"] or not status["connected"]:
        return {"enabled": status["enabled"], "connected": status["connected"], "message": "Neo4j 未启用或未连接。", "hits": []}
    return {"enabled": True, "connected": True, "hits": await kg.search_related(q, limit=10)}
