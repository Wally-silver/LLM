...
export class ApiClient {
  ...
  async upload(file: File) {
    ...
    const res = await fetch(`${this.baseUrl}/datasources/upload`, { method: 'POST', body: form })
    const payload = await res.json()
    if (!res.ok) throw { message: payload?.detail?.message || payload?.message || 'Request failed', detail: payload }
    return payload
  }
  ...
  agent(payload: AskRequest) {
    return this.post<AgentResponse>('/agent', payload)
  }
